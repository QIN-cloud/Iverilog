...
//TF_values �ĸ�ʽΪ <01>([01]* [(case condotion)])*
{
	int i = 1;
	int idx;
...
bool dd_var::add_dd_case_head_node(const char TF_values[DD_DEPTH],int cons,int pro_idx,int r_n,int lineno)
{
	...
	temp->true_ptr = temp1;					//����true��
					
	temp1 = new dd_calc_node;
	temp->false_ptr = temp1;					//����false��
...
// 0  false 1- true
{
	if (this->var_head_ptr == NULL)
	{
		...
		temp1 = new dd_calc_node;
		this->var_head_ptr->true_ptr = temp1;//����true��
				
		temp1 = new dd_calc_node;
		this->var_head_ptr->false_ptr = temp1;//����false��
...
void dd_var::dump_ddnode(dd_node * ddn,ostream &o)
{
	...
	for(int i = 0 ;i < ddn->false_ptr->ri_idx; i++)
