...
svector<Node*>& CfgNode::construct_node(int t)
{
	...
	node->caseitem_ = list<PExpr*>(caseitem_);
	if(assign_type_ != "")
...
void BlockNode::add_node(CfgNode* node, int type)
{
	if(type == 0)
		root_ = node;
	else if(type == 1)
...
GenerateNode::GenerateNode(int ln)
:CfgNode(ln)
{
}
...
void GenerateNode::add_node(ProcessNode* node, int type)
{
	switch(type){
	  case 0:
...
void GenerateNode::add_node(CfgNode* node, int type)
{
	switch(type){
	  case 0:
	    root_ = node;
...
void GenerateNode::add_node(GenerateNode* node)
{
	generates_ = svector<GenerateNode*>(generates_, node);
}

CfgNode* GenerateNode::get_node(int type)
{
    	if(type == 0)
    	  return root_;
    	else 
...
void ModuleNode::add_node(GenerateNode* node)
{
	generates_ = svector<GenerateNode*>(generates_, node);
}
...
svector<GenerateNode*> ModuleNode::get_generatenodes()
{
	return generates_;
}
...
void ModuleNode::dump(ostream&o)
{
	...
	for(idx = 0; idx <assigns_.count(); ++idx)
		assigns_[idx]->dump(o);
	for(idx = 0; idx <generates_.count(); ++idx)
		generates_[idx]->dump(o);
}
...
svector<Node*>& ModuleNode::construct_node(int)
{
	...
	for(idx = 0; idx <generates_.count(); ++idx)
		nodes = new svector<Node*>(*nodes, generates_[idx]->construct_node(2));
	for(idx = 0; idx <assigns_.count(); ++idx)
		nodes = new svector<Node*>(*nodes, assigns_[idx]->construct_node(3));
	return *nodes;
}

void GenerateNode::dump(ostream&o)
{
	if(root_)
	{//cout<<"generate_root"<<endl;
		root_->dump(o);
	}
	/*
	if(st_)
	{cout<<"generate_st"<<endl;
		st_->dump(o);
	}
	*/
	unsigned idx;
...
svector<Node*>& GenerateNode::construct_node(int)
{
	svector<Node*>* nodes = new svector<Node*>();
	unsigned idx;
    if(root_)
...
void CfgNode::Ndump(std::ostream&o, int&nNo)
{
	o << "node_" << ++nNo << "_" << lineno_ << " [";
	o << "shape=box,fontsize=12,color=green,label=";
	o << "\"(" << lineno_ << "," << nNo << ")\"];" <<endl;
}

void BlockNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"block ";
	if(root_)
		root_ -> Ndump(o , nNo);
	for(unsigned idx = 0; idx < nodes_.count(); ++idx)
		nodes_[idx]->Ndump(o, nNo);
}

void CaseNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"case ";
	for(unsigned idx = 0; idx < paths_.count(); ++idx)
		paths_[idx]->Ndump(o, nNo);
}

void CondNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"cond ";
	if(root_)
	{
...
void LoopNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"loop ";
	if(root_)
		root_->Ndump(o, nNo);
...
void ProcessNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"process ";
	if(root_)
		root_->Ndump(o, nNo);
...
void ModuleNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"module ";
	unsigned idx;
	for(idx = 0; idx <procs_.count(); ++idx)
		procs_[idx]->Ndump(o, nNo);
	for(idx = 0; idx <funcs_.count(); ++idx)
		funcs_[idx]->Ndump(o, nNo);
	for(idx = 0; idx <tasks_.count(); ++idx)
		tasks_[idx]->Ndump(o, nNo);
	for(idx = 0; idx <generates_.count(); ++idx)
		generates_[idx]->Ndump(o, nNo);
	for(idx = 0; idx <assigns_.count(); ++idx)
		assigns_[idx]->Ndump(o, nNo);
}

void GenerateNode::Ndump(std::ostream&o, int&nNo)
{
	o<<"generate ";
	unsigned idx;
	for(idx = 0; idx <procs_.count(); ++idx)
...
void CfgNode::Pdump(std::ostream&o, int&pNo)
{
	o << ++pNo << " [";
	o << "shape=ellipse,fontsize=12,color=blue,label=";
	o << "\"";
	for(unsigned idx = 0; idx < expr_.count(); ++idx)
	{
		expr_[idx]->dump(o);
		if(idx != expr_.count()-1)
			o << ", ";
	}
	o << "\"];" <<endl;
}

void BlockNode::Pdump(std::ostream&o, int&pNo)
{
	if(root_)
		root_ -> Pdump(o , pNo);
	for(unsigned idx = 0; idx < nodes_.count(); ++idx)
		nodes_[idx]->Pdump(o, pNo);
}

void CaseNode::Pdump(std::ostream&o, int&pNo)
{
	for(unsigned idx = 0; idx < paths_.count(); ++idx)
		paths_[idx]->Pdump(o, pNo);
}

void CondNode::Pdump(std::ostream&o, int&pNo)
{
	if(root_)
		root_->Pdump(o, pNo);
	if(if_)
...
void LoopNode::Pdump(std::ostream&o, int&pNo)
{
	if(root_)
		root_->Pdump(o, pNo);
	if(st_)
...
void ProcessNode::Pdump(std::ostream&o, int&pNo)
{
	if(root_)
		root_->Pdump(o, pNo);
	if(st_)
...
void ModuleNode::Pdump(std::ostream&o, int&pNo)
{
	unsigned idx;
	for(idx = 0; idx <procs_.count(); ++idx)
		procs_[idx]->Pdump(o, pNo);
	for(idx = 0; idx <funcs_.count(); ++idx)
		funcs_[idx]->Pdump(o, pNo);
	for(idx = 0; idx <tasks_.count(); ++idx)
		tasks_[idx]->Pdump(o, pNo);
	for(idx = 0; idx <assigns_.count(); ++idx)
		assigns_[idx]->Pdump(o, pNo);
}