  ...
  width_(width), width_dist_(width_dist)
{
      ...
      for (idx = 0 ;  idx < width_dist_ ;  idx += 1) {
  ...
  signed_(false), width_r_(wr), width_a_(wa), width_b_(wb), width_s_(ws)
{
      ...
      for (idx = 0 ;  idx < width_s_ ;  idx += 1, p += 1) {
	    pin(p).set_dir(Link::INPUT);
...
/*
NetExpr* NetExpr::dup_expr() const
{
	assert(0);
	return 0;
}
...
NetExpr* NetEBBits::dup_expr() const
{
...
NetExpr* NetEBinary::dup_expr() const
{
      assert(0);
	  return 0;
}
...
NetExpr* NetEBLogic::dup_expr() const
{
...
NetExpr* NetEMemory::dup_expr() const
{
      assert(0);
	  return 0;
}
...
NetExpr* NetEBitSel::dup_expr() const
{
      assert(0);
	  return 0;
}
...
/*
 * $Log: netlist.cpp,v $
 * Revision 1.1  2003/04/08 04:10:48  Administrator
 * no message
 *
 * Revision 1.4  2003/04/01 02:45:26  Administrator
 * no message
 *
 * Revision 1.3  2003/03/19 08:54:45  Administrator
 * no message
 *
 * Revision 1.2  2003/03/18 12:27:39  Administrator
 * no message
 *
 * Revision 1.1  2003/03/16 13:07:39  Administrator
 * no message
 *
