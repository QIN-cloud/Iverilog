...
void PGModule::elaborate_mod_(Design*des, Module*rmod, NetScope*scope) const
{
	...
	// place, then get the binding from the base class.
      if (pins) {
	    ...
	    for (unsigned idx = 0 ;  idx < npins_ ;  idx += 1) {
		  ...
		  unsigned pidx = rmod->find_port(pins[idx].name);
		  ...
		  if (pidx == nexp) {
			cerr << get_line() << ": error: port ``" <<
			      pins[idx].name << "'' is not a port of "
			     << get_name() << "." << endl;
			des->errors += 1;
			continue;
		  }
	...
	// unconnected port.
      ...
      for (unsigned idx = 0 ;  idx < spins->count() ;  idx += 1) {
		...
		&& (prts[0]->port_type() != NetNet::PINPUT)) {
		  ...
		  sig = (*spins)[idx]->elaborate_lnet(des, scope, true);
		  if (sig == 0) {
			cerr << (*spins)[idx]->get_line() << ": error: "
		  ...
		  sig = (*spins)[idx]->elaborate_net(des, scope,
						    prts_pin_count,
						    0, 0, 0);
		  if (sig == 0) {
			cerr << (*spins)[idx]->get_line() << ": error: "
	    ...
	    unsigned spin = 0;
	    for (ldx = prts.count() ;  ldx > 0 ;  ldx -= 1) {
...
NetProc* PCase::elaborate(Design*des, NetScope*scope) const
{
	...
	/* Iterate over all the case items (guard/statement pairs)
      ...
      unsigned inum = 0;
      for (idx = 0 ;  idx < items_->count() ;  idx += 1) {
...
NetProc* PCAssign::elaborate(Design*des, NetScope*scope) const
{
      assert(scope);
...
NetProc* PDeassign::elaborate(Design*des, NetScope*scope) const
{
      assert(scope);
...
NetProc* PWhile::elaborate(Design*des, NetScope*scope) const
{
      NetWhile*loop = new NetWhile(cond_->elaborate_expr(des, scope),
				   statement_->elaborate(des, scope));
	  loop->set_file(get_filename().c_str());
...
bool Module::elaborate(Design*des, NetScope*scope) const
{
	...
	// the signals so that the tasks can reference them.
      ...
      for (mtask_it_t tcur = tasks_.begin()
		 ; tcur != tasks_.end() ;  tcur ++) {

	    NetScope*tscope = scope->child((*tcur).first);
	    assert(tscope);
	    (*tcur).second->elaborate(des, tscope);
      }
	...
	// for each process.
      ...
      for (list<PProcess*>::const_iterator st = sl.begin()
		...
		top->set_id((*st)->get_id());
		
		(*st)->set_synchronous(top->is_synchronous());
