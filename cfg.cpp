...
Cfg* ModuleNode::build_cfg(ProcessNode* pn)
{
		...
		*/
		...
		else
		{
				...
				++ipos)
			{
				...
				{
					if(nodes[index2->second]->caseitem_.count() > 0)
					{
						for(unsigned eidx = 0; eidx < nodes[index2->second]->caseitem_.count(); ++eidx)
							dsctmp2->caseitem.insert(nodes[index2->second]->caseitem_[eidx]);
					}
					else //default
						dsctmp2->cond = false;
	...
	for(unsigned nidx = 0; nidx < cfg->root->count(); ++nidx)
	{
		...
		for(spos = (*(cfg->root))[nidx]->refs.begin(); spos != (*(cfg->root))[nidx]->refs.end(); ++spos)
		{
